...
execute if score #compact_ids tpa.config matches 1 run scoreboard players set @a[scores={tpa.available=0}] tpa.player_id -1
execute if score #compact_ids tpa.config matches 0 run scoreboard players operation @a[scores={tpa.available=0}] tpa.player_id *= #-1 tpa.variables

# Offline detect: {Player count decreased ? Remove offline Player ids : do nothing}
execute store result score #current_avail_count tpa.variables if entity @a[scores={tpa.available=1}]
...
execute as @a[scores={tpa.available=1}] run scoreboard players operation #sum_of_id tpa.variables += @s tpa.player_id

# Give spectators and fake players whose game id was shaderred a new id
execute if score #compact_ids tpa.config matches 1 as @a[scores={tpa.available=1, tpa.player_id=..-1}] run scoreboard players set @s tpa.player_id 1
execute if score #compact_ids tpa.config matches 0 as @a[scores={tpa.available=1, tpa.player_id=..-1}] run scoreboard players operation @s tpa.player_id = @s tpa.uid
