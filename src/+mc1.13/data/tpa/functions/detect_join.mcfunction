...
execute unless score #language tpa.config matches 1.. run tellraw @s ["[§bTPA§r] 检测到默认语言未设置, 请点击下方设置TPA数据包的服务器默认语言 | Detected Default language is not set, please click below to set default server language of TPA datapack:  ", {"text": "[§a设置 | Set§r]", "clickEvent": {"action": "run_command", "value": "/trigger tpa.language set -1"}}]
...
# Set player ID
execute if score #compact_ids tpa.config matches 0 as @s[scores={tpa.available=1}] run scoreboard players operation @s tpa.player_id = @s tpa.uid
execute if score #compact_ids tpa.config matches 1 as @s[scores={tpa.available=1}] run scoreboard players set @s tpa.player_id 1

# Show TPA menu button when player joins
tellraw @s[scores={tpa.output_=1..2}] [{"text": "[§6点我打开TPA菜单§r]", "clickEvent": {"action": "run_command", "value": "/trigger tpa"}}]
