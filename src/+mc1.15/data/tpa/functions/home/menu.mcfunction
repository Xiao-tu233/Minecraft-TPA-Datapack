...
execute if score #debug_mode tpa.config matches 1 run tellraw @a ["[§bTPA§r] §6 Debug: §rGot home profile for uid of ",{"score": {"name": "#uid","objective": "tpa.variables"}, "color": "green"}, " : ", {"storage": "tpa:tpa", "nbt": "temp.home"}]

# Sort temp.home
function tpa:home/menu/sort
data modify storage tpa:tpa temp.home set from storage tpa:tpa temp.home_sorted

execute if score #debug_mode tpa.config matches 1 run tellraw @a ["[§bTPA§r] §6 Debug: §rSorted: ", {"storage": "tpa:tpa", "nbt": "temp.home"}]
