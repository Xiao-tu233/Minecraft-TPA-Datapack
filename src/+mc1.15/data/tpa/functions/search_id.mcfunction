...
execute if score #search_id tpa.config matches 1 run tellraw @s[predicate=tpa:output/show_chatbar] [{"storage": "tpa:tpa", "nbt": "loaded_lang.header"}, {"storage": "tpa:tpa", "nbt": "loaded_lang.search_id_disabled"}, ". " ]
