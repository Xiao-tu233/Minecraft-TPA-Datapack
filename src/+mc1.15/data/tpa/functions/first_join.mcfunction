...
execute if score #can_choose tpa.variables matches 1 run tellraw @s {"storage": "tpa:tpa","nbt": "temp.lang.lang_button","clickEvent":{"action":"run_command","value":"/trigger tpa.language set -1"},"hoverEvent":{"action":"show_text","value":{"storage": "tpa:tpa", "nbt": "temp.lang.lang_button_hoverevent"}}}

# Give UID
scoreboard players operation @s tpa.uid = #global_current tpa.uid
scoreboard players add #global_current tpa.uid 1
execute if score #debug_mode tpa.config matches 1 run tellraw @a ["[§bTPA§r] §6 Debug: ", {"selector": "@s"}, " joined the server for the first time, who has now UID of ", {"color": "green","score": {"name": "@s", "objective":"tpa.uid"}}]
