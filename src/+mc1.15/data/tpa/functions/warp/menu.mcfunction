...
# Storage Format:
# {desc: string, name: string, x: int, y: int, z: int, dimension: Dimension, disabled: boolean}
...
execute if score #is_editting_warp tpa.variables matches 1 run tellraw @s ["  ", {"storage": "tpa:tpa", "nbt": "warp[0].warp_button_add","hoverEvent":{"action":"show_text","value":{"storage": "tpa:tpa", "nbt": "loaded_lang.warp_button_add_hoverevent"}},"clickEvent":{"action":"run_command","value":"/function tpa:warp/setpos {index: $(index)}"}}]
