...
execute at @s as @e[type=item, name='Written Book', distance=..2] if data entity @s Item.components."minecraft:custom_data".isTpaBook run function tpa:book/refresh

# Book count = clear(book, count=0)
scoreboard players set #book_count tpa.variables 0
execute store result score #book_count tpa.variables run clear @s minecraft:written_book[minecraft:custom_data={isTpaBook:1b}] 0
