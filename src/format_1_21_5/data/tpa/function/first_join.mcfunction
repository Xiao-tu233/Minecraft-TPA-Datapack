...
execute if score #can_choose tpa.variables matches 1 run tellraw @s {storage: "tpa:tpa",nbt: "loaded_lang.lang_button",click_event:{action:"run_command",command:"/trigger tpa.language set -1"},hover_event:{action:"show_text",value:{storage: "tpa:tpa", nbt: "loaded_lang.lang_button_hoverevent"}}}
