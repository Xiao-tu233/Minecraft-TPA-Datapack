...
execute if score #is_recver_found tpa matches 1 run tellraw @p[tag=to_modify] ["[§bTPA§r] ",{selector:"@s",hover_event:{action:"show_text",value:"§b向你发送了请求"}},"传送到了你这里"]
execute if score #is_recver_found tpa matches 1 run tellraw @s ["[§bTPA§r] 传送你至",{selector:"@p[tag=to_modify]",hover_event:{action:"show_text",value:"§b你传送的玩家"}},"..."]
