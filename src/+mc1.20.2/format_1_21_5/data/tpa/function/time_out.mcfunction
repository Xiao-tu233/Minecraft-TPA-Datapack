...
execute if score #is_recver_found tpa.variables matches 1..2 \
    run tellraw @s[predicate=tpa:output/show_chatbar] [{storage:"tpa:tpa", nbt:"loaded_lang.header"}, \
        {storage:"tpa:tpa", nbt:"loaded_lang.reqer_timeout_left_part"}, \
        {selector:"@p[tag=to_modify]",hover_event:{action:"show_text",value:{storage:"tpa:tpa", nbt:"loaded_lang.reqer_timeout_hoverevent"}}}, \
        {storage:"tpa:tpa", nbt:"loaded_lang.reqer_timeout_right_part"}, ". " \
    ]
execute if score #is_recver_found tpa.variables matches 1..2 \
    run title @s[predicate=tpa:output/show_actionbar] actionbar [\
        {storage:"tpa:tpa", nbt:"loaded_lang.reqer_timeout_left_part", color: "gold"}, \
        {selector:"@p[tag=to_modify]"}, \
        {storage:"tpa:tpa", nbt:"loaded_lang.reqer_timeout_right_part", color: "gold"} \
    ]
execute if score #is_recver_found tpa.variables matches 1..2 \
    run tellraw @p[tag=to_modify, predicate=tpa:output/show_chatbar] [{storage:"tpa:tpa", nbt:"loaded_lang.header"}, \
        {storage:"tpa:tpa", nbt:"loaded_lang.recver_timeout_left_part"}, \
        {selector:"@s",hover_event:{action:"show_text",value:{storage:"tpa:tpa", nbt:"loaded_lang.recver_timeout_hoverevent"}}}, \
...
execute if score #is_recver_found tpa.variables matches 1..2 \
    run title @p[tag=to_modify, predicate=tpa:output/show_actionbar] actionbar [\
        {storage:"tpa:tpa", nbt:"loaded_lang.recver_timeout_left_part", color: "gold"}, \
        {selector:"@s"}, \
        {storage:"tpa:tpa", nbt:"loaded_lang.recver_timeout_right_part", color: "gold"} \
    ]
