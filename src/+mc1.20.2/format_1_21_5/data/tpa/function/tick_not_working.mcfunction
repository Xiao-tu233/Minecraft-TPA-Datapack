...
data modify entity @n[tag=tpa.text_display] text set value '["[TPA] ", {storage: "tpa:tpa", nbt: "loaded_lang.tick_not_working_server_calling"}]'
...
tellraw @a [{storage: "tpa:tpa", nbt: "loaded_lang.tick_not_working_tips_line2"}, " ", {storage: "tpa:tpa", nbt: "loaded_lang.tick_not_working_button_set_schedule", hover_event: {action: "show_text", value: {storage: "tpa:tpa", nbt: "loaded_lang.tick_not_working_button_set_schedule_hoverevent"}}, click_event: {action:  "run_command", command: "/scoreboard players set #uses_tick_scheduling tpa.config 1"}}]
