...
execute if score #command_len tpa.variables matches 1 run tellraw @s[predicate=tpa:output/show_chatbar] [\
    ...
    {interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.reqer_req_not_exist"}, "." \
]
execute if score #command_len tpa.variables matches 1 run title @s[predicate=tpa:output/show_actionbar] actionbar [{interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.reqer_req_not_exist"}]
...
execute if score #has_illegal_char tpa.variables matches 1 run tellraw @s[predicate=tpa:output/show_chatbar] [\
    {interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.header"}, \
    {interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.search_id_throw_on_invalid_char"}, "." \
]
execute if score #has_illegal_char tpa.variables matches 1 run title @s[predicate=tpa:output/show_actionbar] actionbar [{interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.search_id_throw_on_invalid_char"}]
...
execute unless score #tp_to tpa.variables matches 2.. run title @s[predicate=tpa:output/show_actionbar] actionbar [{interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.search_id_unavail_player"}]
