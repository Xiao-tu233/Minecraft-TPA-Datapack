...
execute if score #debug_mode tpa.config matches 1 run tellraw @a ["[§bTPA§r] §6 Debug§r: Missing Book Discovered: in Entity Container ", {selector: "@s"}, " at ", {interpret: true, entity: "@s", nbt: "Pos[]", "separator": ","}, ", Slot ", {interpret: true, storage: "tpa:tpa", nbt: "temp.book.merger.Slot"}, ", count ", {interpret: true, storage: "tpa:tpa", nbt: "temp.book.merger.count"}]
