...
execute if score #debug_mode tpa.config matches 1 run tellraw @a ["[§bTPA§r] §6 Debug§r: Missing Book Discovered: in Block Container ", {interpret: true, storage: "tpa:tpa", nbt: "temp.args.x"}, " ", {interpret: true, storage: "tpa:tpa", nbt: "temp.args.y"}, " ", {interpret: true, storage: "tpa:tpa", nbt: "temp.args.z"}, "(", {interpret: true, storage: "tpa:tpa", nbt: "temp.book.checking_block"}, ") at ", {interpret: true, entity: "@s", nbt: "Pos[]", "separator": ","}, ", Slot ", {interpret: true, storage: "tpa:tpa", nbt: "temp.book.merger.Slot"}, ", count ", {interpret: true, storage: "tpa:tpa", nbt: "temp.book.merger.count"}]
