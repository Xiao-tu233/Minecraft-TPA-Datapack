...
# Show title
execute unless score #is_server_selecting tpa.variables matches 1 run tellraw @s[predicate=tpa:output/show_chatbar] [{interpret: true, storage: "tpa:tpa", nbt: "loaded_lang.header"}, {interpret: true, storage: "tpa:tpa", nbt: "loaded_lang.lang_menu_title"}, "\n", {interpret: true, storage: "tpa:tpa", nbt: "loaded_lang.lang_menu_select_button",click_event:{action:"suggest_command",command: "/trigger tpa.language set " }, hover_event: {action: "show_text", value: {interpret: true, storage: "tpa:tpa", nbt: "loaded_lang.lang_menu_cancel_button_hoverevent"} } } ]
execute if score #is_server_selecting tpa.variables matches 1 run tellraw @s [{interpret: true, storage: "tpa:tpa", nbt: "loaded_lang.header"}, {interpret: true, storage: "tpa:tpa", nbt: "loaded_lang.lang_menu_title"}, "\n", {interpret: true, storage: "tpa:tpa", nbt: "loaded_lang.lang_button_server",click_event:{action:"suggest_command",command: "/scoreboard players set #language tpa.config " }, hover_event: {action: "show_text", value: {interpret: true, storage: "tpa:tpa", nbt: "loaded_lang.lang_menu_select_button_hoverevent"} } } ]
