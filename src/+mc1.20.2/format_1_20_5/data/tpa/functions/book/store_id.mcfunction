...
$data modify entity @e[sort=nearest, limit=1, type=minecraft:text_display,tag=tpa.text_display] text set value '{"storage":"tpa:tpa","nbt":"book.each_page.id_$(key)"}'
$data modify storage tpa:tpa book.each_page.id_$(key) set from entity @e[sort=nearest, limit=1, type=minecraft:text_display,tag=tpa.text_display] text
