...
execute if score #debug_mode tpa.config matches 1 run tellraw @a {translate: "[§bTPA§r] §6 Debug: §rRequest_%1$s from %2$s to %3$s removed due to TIME OUT", with: [{score: {name: "#direction", objective: "tpa.variables"}}, {score: {name: "#requests.current_reqer", objective: "tpa.variables"}}, {score: {name: "#requests.current_recver", objective: "tpa.variables"}}]}
