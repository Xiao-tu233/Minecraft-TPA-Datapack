...
tellraw @s[predicate=tpa:output/show_chatbar] [{interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.header"}, \
    {interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.simple_menu_title"}, \
    {\
        storage:"tpa:tpa", \
        nbt:"loaded_lang.simple_menu_button", \
        click_event:{action:"run_command",command: "/trigger tpa.simple_menu set 3"}, \
        hover_event: {action: "show_text", value: {interpret: true, storage:"tpa:tpa", nbt:"loaded_lang.simple_menu_button_hoverevent"}}\
    }, " " \
] 
...
execute if score #debug_mode tpa.config matches 1 run tellraw @a ["[§bTPA§r] §6 Debug: §rNow showing: §astorage tpa:tpa temp.simple_menu.result:", {interpret: true, storage:"tpa:tpa", nbt:"temp.simple_menu.result"}]
