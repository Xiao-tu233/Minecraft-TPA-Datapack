...
scoreboard players set #request_selector.removing tpa.variables 1
...
execute if score #debug_mode tpa.config matches 1 run tellraw @a ["[TPA] Debug: Matched Receive Selector Removed: ", {storage: "tpa:tpa", nbt: "temp.matched_selector"}]
