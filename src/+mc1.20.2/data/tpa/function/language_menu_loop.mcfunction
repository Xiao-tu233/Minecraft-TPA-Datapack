execute if data storage tpa:tpa loaded_language[0] unless score #is_server_selecting tpa.variables matches 1 \
    ...
    {\
        ...
        hover_event: {action: "show_text", value: {interpret: true, storage: "tpa:tpa", nbt: "loaded_language[0].name"}}\
    } \
]
...
execute if data storage tpa:tpa loaded_language[0] run function tpa:language/dialog_append_player with storage tpa:tpa loaded_language[0]
