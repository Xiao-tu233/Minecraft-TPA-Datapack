...
execute if score #debug_mode tpa.config matches 1 if data storage tpa:tpa temp.back_result run \
    tellraw @a ["[§bTPA§r] §6 Debug§r: Back extract: found for ", {selector: "@s"}, {translate: "(UID:§a%s§r)", with: [{score: {name: "#uid", objective: "tpa.variables"}}]}]
execute if score #debug_mode tpa.config matches 1 unless data storage tpa:tpa temp.back_result run \
    tellraw @a ["[§bTPA§r] §6 Debug§r: Back extract: not found for ", {selector: "@s"}, {translate: "(UID:§a%s§r)", with: [{score: {name: "#uid", objective: "tpa.variables"}}]}]