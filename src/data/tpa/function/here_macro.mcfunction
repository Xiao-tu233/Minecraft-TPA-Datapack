...
$execute unless score #player_available tpa.variables matches 1 run tellraw @a[predicate=tpa:output/show_chatbar] [\
    ...
    {"storage": "tpa:tpa", "nbt": "loaded_lang.here_button_tpa", "color": "gray", "hoverEvent": {"action": "show_text", "value": {"storage": "tpa:tpa", "nbt": "loaded_lang.here_button_tpa_notavail_hoverevent"}}}, " ", \
    {"text": "[§b+V§r]", "clickEvent": {"action": "run_command", "value": "/newWaypoint x:$(x), y:$(y), z:$(z), dim:$(dimension)"}, "hoverEvent": {"action": "show_text", "value": {"storage": "tpa:tpa", "nbt": "loaded_lang.here_voxel_hoverevent"}}}, " ", \
    {"text": "[§6+X§r]", "clickEvent": {"action": "suggest_command", "value": "xaero_waypoint_add:$(name):$(n):$(x):$(y):$(z):6:false:0:Internal_$(dimension)_waypoints"}, "hoverEvent": {"action": "show_text", "value":{"storage": "tpa:tpa", "nbt": "loaded_lang.here_xaero_hoverevent"}}}, " " \
]
...
$execute if score #player_available tpa.variables matches 1 run tellraw @a[predicate=tpa:output/show_chatbar] [\
    ...
    {"storage": "tpa:tpa", "nbt": "temp.args.y_trans", "color": "$(trans_color)"}, {"storage": "tpa:tpa", "nbt": "temp.args.space_trans"}, \
    {"storage": "tpa:tpa", "nbt": "temp.args.z_trans", "color": "$(trans_color)"}, " ", \
    {"storage": "tpa:tpa", "nbt": "loaded_lang.here_button_tpa", "clickEvent": {"action": "run_command", "value": "/trigger tpa.tpa set $(id)"}, "hoverEvent": {"action": "show_text", "value": {"storage": "tpa:tpa", "nbt": "loaded_lang.here_button_tpa_hoverevent"}}}, " ", \
    {"text": "[§b+V§r]", "clickEvent": {"action": "run_command", "value": "/newWaypoint x:$(x), y:$(y), z:$(z), dim:$(dimension)"}, "hoverEvent": {"action": "show_text", "value": {"storage": "tpa:tpa", "nbt": "loaded_lang.here_voxel_hoverevent"}}}, " ", \
    {"text": "[§6+X§r]", "clickEvent": {"action": "suggest_command", "value": "xaero_waypoint_add:$(name):$(n):$(x):$(y):$(z):6:false:0:Internal_$(dimension)_waypoints"}, "hoverEvent": {"action": "show_text", "value":{"storage": "tpa:tpa", "nbt": "loaded_lang.here_xaero_hoverevent"}}}, " " \
]
