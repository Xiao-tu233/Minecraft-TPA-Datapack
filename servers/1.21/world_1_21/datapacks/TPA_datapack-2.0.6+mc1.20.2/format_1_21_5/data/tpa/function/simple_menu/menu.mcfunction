...
tellraw @s[scores={tpa.output=1..2}] [{storage:"tpa:tpa", nbt:"temp.lang.header"}, \
    ...
    {\
        ...
        hover_event: {action: "show_text", value: {storage:"tpa:tpa", nbt:"temp.lang.simple_menu_button_hoverevent"}}\
    }, " " \
] 

# Remove the ', {color:"dark_gray", text: " | "}, ', and trailing comma, by C style: result = result[40:]
data modify storage tpa:tpa temp.simple_menu.result[0] set value ''
...
tellraw @s[scores={tpa.output=1..2}] [{storage:"tpa:tpa", nbt:"temp.simple_menu.result[]", interpret: 1b, separator: ''}]
