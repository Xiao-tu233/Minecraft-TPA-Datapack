#> tpa:req/reqer_here.mcfunction
execute if score #debug_mode tpa_config matches 1 run tellraw @a "[§bTPA§r] §6 Debug: §rNow executing: §atpa:reqer_here"
...
execute if score #back tpa_config matches 1 run function tpa:store_pos

#act tp
tp @s @p[tag=to_modify]
scoreboard players set @p[tag=to_modify] tp_to 0
function tpa:langtell/langtell_34
execute as @p[tag=to_modify] run function tpa:langtell/langtell_64
