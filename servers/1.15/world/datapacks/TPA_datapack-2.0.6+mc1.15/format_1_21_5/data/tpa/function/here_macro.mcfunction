...
$execute if score #dim_num tpa.variables matches 0 run tellraw @a[scores={tpa.output=1..2}] [\
    ...
    {storage: "tpa:tpa", nbt: "temp.lang.here_button_tpa", click_event: {"action": "run_command", "value": "/trigger tpa.tpa set $(id)"}, hover_event: {"action": "show_text", "value": {storage: "tpa:tpa", nbt: "temp.lang.here_button_tpa_hoverevent"}}}, " ", \
    {text: "[§b+V§r]", click_event: {"action": "run_command", "value": "/newWaypoint x:$(x), y:$(y), z:$(z), dim:$(dimension)"}, hover_event: {"action": "show_text", "value": {storage: "tpa:tpa", nbt: "temp.lang.here_voxel_hoverevent"}}}, " ", \
    {text: "[§6+X§r]", click_event: {"action": "suggest_command", "value": "xaero_waypoint_add:$(name):$(n):$(x):$(y):$(z):6:false:0:Internal_$(dimension)_waypoints"}, hover_event: {"action": "show_text", "value":{storage: "tpa:tpa", nbt: "temp.lang.here_xaero_hoverevent"}}}, " " \
]
