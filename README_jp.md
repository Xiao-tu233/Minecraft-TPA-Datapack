...
* インストールは非常に簡単です：資料パックを使用します。資料パックは Minecraft に簡単に導入できるコンテンツ形式で、拡張やローダーは不要です。
* 他の資料パック／MOD／プラグイン／資源包との互換性も積極的に確認しています。そのため、TPA 資料パックが原因でない可能性のあるバグでも、再現方法とともに報告可能です。
...
下表で、Minecraft のバージョンに対応する資料パックのファイルサフィックスを確認してください。
**Minecraft バージョンと資料パックファイルサフィックス対応表**
...
他の資料パックと基本的に同じです。すでに知っている場合は読み飛ばしてください。
...
4. 資料パックファイルを `datapacks` フォルダに入れます。
...
8. `datapacks` フォルダに資料パックファイルを入れます。
9. 世界に入り、`/datapack list` で認識されていることを確認します。

### 新しい世界を作成する場合

1. `その他 -> 資料パック -> パックフォルダを開く` をクリックします。
2. 資料パックファイルを入れます。
3. 世界を作成します。
